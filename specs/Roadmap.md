...
| Commit | 说明 |
|--------|------|
| `4a59f12` | App Store 首次 Transporter 上传被拒（错误 `-19239`）：`CFBundleVersion` 为 `3550659-d.0721-1501`，包含 Git hash、dirty 标记和连字符，不符合 1–3 段纯整数要求。根因是 `release-appstore.sh` 复用了站外 DMG 的开发者可读构建号。修复为 App Store 专用 14 位纯数字时间构建号，并在导出前校验最终归档内的 `CFBundleVersion` 格式及传入值一致性；站外 DMG 版本逻辑保持不变。 |
