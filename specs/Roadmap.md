...
| ✅ warm 已解决 / cold 待 Slice2 | OpenWith warm 激活 | **warm open 窗口不自动置顶**——原 V1 模型（复用图库主窗）下 macOS 14 拒绝 app 自消费 open 事件时抢前台，5 种激活 API 真机全败（旧 backlog）。**2026-06-04 方向2 Slice1 真机验证：warm 置顶成功** —— 自建独立 `ExternalViewerWindowController`（不复用主窗，commit `7a32dff` + 1×1/focus fix `a3e4ae0`）后置顶可控，方向2 核心赌注成立，本顽疾 warm 部分**解决**。**剩余同根缺陷（SwiftUI `Window` scene 仍在场，Slice1 硬边界保留）→ Slice2 一并根治**：(a) **cold 启动双窗**——app 未运行时 Finder 打开方式会同时拉起图库主窗 + 看图窗（SwiftUI 主 scene 无条件建主窗）；(b) **warm 看完关图后图库主窗不自动回来**——odoc 进来时 SwiftUI 主 scene 处理 open-document 把主窗瞬态 close（窗口数→0，`shouldTerminate=false` 防自杀），关看图窗后须点 Dock reopen 才重建主窗。两者根因都是 `Window("一眼")` scene 还在，Slice2 移除它 + AppDelegate 用 `MainWindowController` 自建主窗（D-OW9/10）后一并消失。设计/plan 见 `specs/2026-06-03-openwith-lightweight-viewer-{design,plan}.md`，决策见上方 D-OW5~11（#34-37） |
