...
| `<本次>` | 图片加载失败显占位而非无限 loading（方案 3，Slice 2）：grid cell / 预览 / QV 三处 image nil 既表"加载中"又表"失败"→ 无限转圈。修：新建共享 `ImageLoadFailedView`（compact 模式 grid cell 仅图标）+ 三处加 `loadFailed` 标志（SmartFolderImageCell / ImagePreviewViewModel / QuickViewerViewModel），load 结束 image 仍 nil → 显占位。兜真删磁盘文件 FSEvents race / 损坏文件 / 不支持格式。附带：`loadFullNSImage` / `loadThumbnail` 标 `nonisolated`——项目 default main-actor isolation 把这俩离主线程解码 helper 当 @MainActor，Task.detached 里调会 hop 回主线程解码（UI 卡）；标 nonisolated 既消 actor warning 又恢复真·后台解码 |
