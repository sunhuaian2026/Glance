...
| `b1c10f9` | 删 `loadThumbnail` 的 `maxPixelSize: Int = 200` 魔法数字默认值（codex pre-push P1）。三个 caller 全传显式值（targetSize*scale / size*scale / DS.Viewer.filmstripThumbLoadSize），默认从未被用 → 删默认变必传参，无需新常量。该行因 Slice 2 给 loadThumbnail 加 nonisolated 进入 diff 才被复审到 |
