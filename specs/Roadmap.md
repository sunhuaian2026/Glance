...
| backlog / 难修 | OpenWith warm 激活 | **warm open（Glance 运行中、Finder「打开方式」打开图）窗口不自动置顶**——图打开了、QV 显示着、不崩，但窗口留在 Finder 之后（用户需点 Dock/窗口调前）。根因：macOS 14 下 app 自己消费 open 事件（AppDelegate.application(open:)）+ 单 Window scene 瞬态 close/reopen，系统拒绝该 app 抢前台。试过 5 种激活 API（NSApp.activate cooperative/forceful、NSRunningApplication.activate+didBecomeActive）真机全败（codex:rescue 三轮 review）。当前 best-effort（orderFrontRegardless 兜底）。**功能不受影响,仅置顶 polish 缺失。** 待有 GUI 调试/截图回传手段再啃；候选根治方案：放弃 AppDelegate 消费、改走 SwiftUI `onOpenURL` 原生通道让系统自然 front（未验证 onOpenURL 是否对 file odoc 触发）。开发 Mac mini 无 GUI 会话故 CC 无法自验此类 GUI 行为 |
...
|------|------|------|--------|----------|------|
| ✅ 已完成 `<本次>` | OpenWith Slice 2 浏览所在文件夹 | `specs/OpenWith.md` + `specs/OpenWith-plan.md` Task 2.x | — | — | QV「浏览所在文件夹」按钮 + handleBrowseFolder 已 ship。OpenWith 功能（Slice 1 看单图 + Slice 2 浏览文件夹）完整 |
