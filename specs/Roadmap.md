...
| `<本次>` | 侧边栏移除文件夹后智能文件夹仍残留缩略图 + 点开一直 loading（方案 1 移除即清理，Slice 1）：根因 `FolderStoreIndexBridge.sync` 移除 diff 用内存 `registeredPaths`（启动空 → 从不对账清残留）。修：(1) `BookmarkManager.managedRootPaths()` 同步受管根权威集；(2) `IndexStore.fetchRootPaths()`（parent_root_id IS NULL AND root_path IS NOT NULL）+ `deleteOrphanImages()`（NOT EXISTS 防御性孤儿清扫）；(3) `sync` 移除段改 **DB+bookmark 权威对账（Guard B）**——删 DB 里不在 managedRootPaths 的 root（FK CASCADE 连删 images），不用异步滞后的 rootFolders（启动瞬态空集会误删整库），managedRootPaths 同步永不滞后能区分"瞬态空"vs"删到最后一个"；(4) ContentView 两处 sync 传 managedRootPaths。codex:rescue 设计 review 确认 Guard B 优于 Guard A。**方案 3（加载失败占位）= Slice 2 另提交。** 加载失败一直 loading 的兜底未做 |
