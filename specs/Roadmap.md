...
| backlog / 难修 | OpenWith warm 激活 | **warm open（Glance 运行中、Finder「打开方式」打开图）窗口不自动置顶**——图打开了、QV 显示着、不崩，但窗口留在 Finder 之后（用户需点 Dock/窗口调前）。根因：macOS 14 下 app 自己消费 open 事件（AppDelegate.application(open:)）+ 单 Window scene 瞬态 close/reopen，系统拒绝该 app 抢前台。试过 5 种激活 API（NSApp.activate cooperative/forceful、NSRunningApplication.activate+didBecomeActive）真机全败（codex:rescue 三轮 review）。当前 best-effort（orderFrontRegardless 兜底）。**功能不受影响,仅置顶 polish 缺失。** 待有 GUI 调试/截图回传手段再啃；候选根治方案：放弃 AppDelegate 消费、改走 SwiftUI `onOpenURL` 原生通道让系统自然 front（未验证 onOpenURL 是否对 file odoc 触发）。开发 Mac mini 无 GUI 会话故 CC 无法自验此类 GUI 行为。**2026-06-03 方向2 重构 in progress**：Slice1（自建独立看图窗 `ExternalViewerWindowController`，不复用图库主窗 → 自建 NSWindow 置顶可控，commit `<pending>`）已 ship 待真机验 warm 置顶；Slice2 收 lifecycle 后预期根治本顽疾。设计/plan 见 `specs/2026-06-03-openwith-lightweight-viewer-{design,plan}.md`，决策见上方 D-OW5~11（#34-37） |
