...
| 状态 | 模块 | 问题描述 | 已知信息 |
|------|------|----------|----------|
| 方案 1 已修 `0a069e6` / 方案 3 待做 | IndexStore + SmartFolder | 侧边栏移除文件夹（非物理删除）后，该文件夹的图仍残留在智能文件夹缩略图里，点开预览/QV 一直 loading。**方案 1（移除即清理）已 ship（`0a069e6` Slice 1，DB+bookmark 权威对账 + 孤儿清扫）→ 残留缩略图问题已解。剩方案 3（加载失败显占位而非无限 loading）= Slice 2 待做**，兜真删磁盘文件 / 解码失败 / FSEvents race 三处（grid cell / preview / QV）。下方原始诊断保留：根因：`FolderStoreIndexBridge.sync` 的移除 diff 用内存 `registeredPaths`（启动时空）→ 启动从不对账清理 DB 残留 root；smart folder 查询 `FROM images` 不强制 JOIN folders → 残留 image 行照查出 → 缩略图显示；预览/QV 走 `computeV2Urls` 无 startAccessing 而移除时已 stopAccessing 撤 scope → 读不到一直 loading。**方案（用户拍板 1+3）**：(1) `sync` 移除 diff 改 DB 权威对账（`fetchRootPaths` vs incoming → deleteRoot FK CASCADE），**必须防瞬态空 incoming 误删整个索引**（加 didRestoreFolders 类 guard，过 codex review）；(3) 加载失败显占位提示而非无限 loading（grid cell / preview / QV 三处）。下一摊独立 commit |
