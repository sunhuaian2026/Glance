...
| Commit | 说明 |
|--------|------|
| QV toolbar 方案2 S1+S2 `ebd88bf`~`a4520c5` | **QV/全屏 toolbar regression（方案2 独立看图窗）**：方向2 Slice2 后主窗 NSHostingView 让 SwiftUI 持续复写 `.toolbar{}` 回 titlebar、看图不纯净。修：QV 从 ContentView `.overlay` 迁 `MainQuickViewerWindowController` 独立无装饰窗。**Slice1 windowedCover**（`ebd88bf`~`4cb1323`，9 项真机验过：titlebar 纯净/同框跟随/焦点归还/找类似·⌘F/状态保留/全屏 ESC）+ **Slice2 全屏**（`5d47b3c`~`a4520c5`，3 态状态机 + enteredFromMainFullScreen flag；inheritedMain 主窗全屏进 QV 走 codex 候选2 = QV 自己原生全屏新 Space，真机验过满屏 + ESC 退回主窗全屏 grid；候选1 borderless+fullScreenAuxiliary 满屏覆盖踩 3 坑弃用）。详见关键架构决策第 11 条 + `specs/v2/2026-06-08-qv-toolbar-*`。Slice3 边界硬化进行中。 |
