...
| `<pending>` | OpenWith 方向2 Slice1 真机验发现 2 bug + 1 调整（详见 OpenWith plan Slice 1 完成详细）：(1) **看图窗只显 1×1 像素、看不到图**（cold/warm 都中，用户报"主界面闪一下→看不到 QV→app 僵尸态点 Dock 无反应"）→ `createWindow` 误用 `contentViewController = NSHostingController`，AppKit 忽略 contentRect 改用 hosting `fittingSize`（初始 `EmptyView`=0）→ 窗口压成 1×1；改 `contentView = NSHostingView`（mirror `AboutWindowController` 已验证骨架）+ `autoresizingMask=[.width,.height]` 跟随 resize/全屏；(2) **看图窗首开键盘须先点鼠标**（ESC/F/方向键不响应）→ `isWindowKey` 在 QV mount 前就翻 true、`.onChange` 漏触发，仅剩 onAppear 那次 assert 被刚 mount 帧丢弃 → `requestKeyboardFocusIfWindowIsKey()` 下一 runloop 补一次 `isFocused`（QuickViewerOverlay 共享改动，主窗用默认行为无副作用）；(3) 交通灯保持隐藏（QV 自带 X 关窗，红绿灯冗余且丑，revert 掉中途加的 `managesHostTrafficLights`）。**真机验过：warm 置顶成功（方向2 核心赌注成立）/ 多图翻页 / 连换图不显旧图 / focus / ESC·⌘W·X 关窗 app 不退**。诊断手段：NSLog 经统一日志被 redact 成 `<private>`，改写沙盒文件 trace 定位（诊断埋点已清理） |
