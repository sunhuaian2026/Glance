...
- [ ] (2026-06-03 / `<本次>` / OpenWith Slice 1) **Dock 拖放接收**：把图片文件拖到 Dock 的 Glance 图标 → 应进 QuickViewer 看该图（同 application(_:open:) 路径，未单独实测）
- [ ] (2026-06-03 / `<本次>` / OpenWith Slice 1) **多图 QV 集合导航**：多图「打开方式」进 QV 后，左右方向键 / filmstrip 点击能在选中集合内切换
...
- [x] (2026-06-03 / `<本次>`) **多图「打开方式」单窗口**：Finder 选多图 → 打开方式 → Glance → 只有一个窗口直接进 QV（不再 spawn 多窗口，Mission Control 确认）✓ 2026-06-03
- [x] (2026-06-03 / `<本次>`) **外部打开 ESC 直接关 QV**：进 QV 后不点图直接 ESC 立刻关（焦点修复）✓ 2026-06-03
- [x] (2026-06-03 / `<本次>`) **回归 — grid / preview 双击进 QV ESC 直接关**：老路径焦点未被 isWindowKey 改动破坏 ✓ 2026-06-03
- [x] (2026-06-03 / `<本次>`) **智能文件夹进 QV tooltip 首次悬停正确**：底部工具栏首次悬停显正确文案，不再串成随机文件名 ✓ 2026-06-03
- [x] (2026-06-03 / `<本次>`) **smart folder cell tooltip 显完整路径**：hover cell 显示图片完整磁盘路径 ✓ 2026-06-03
