...
### V2 M3 Slice M — 焦点 + 回车交互修复（待真机验，`<pending>`）

- [ ] (2026-06-04 / `<pending>` / Slice M fix) **⌘F 焦点自动落 input**：任意位置按 ⌘F → 搜索框弹出 → **不点鼠标直接打字就能输入**（修焦点竞争 bug）
- [ ] (2026-06-04 / `<pending>` / Slice M fix) **回车进结果网格**：输入关键字（实时出结果）→ 回车 → 搜索框收起、结果留在屏幕、焦点落结果网格（**第一张有紫色高亮**）
- [ ] (2026-06-04 / `<pending>` / Slice M fix) **网格内回车/空格开图**：回车进网格后，方向键移高亮 → 回车**或**空格 → 打开当前高亮图进 QV；ESC → 退回普通 grid
- [ ] (2026-06-04 / `<pending>` / Slice M fix) **空输入回车 no-op**：搜索框空着按回车 → 什么都不发生，overlay 留着（不塌成空结果）
- [ ] (2026-06-04 / `<pending>` / Slice M fix) **回归 — M2 找类似不变**：QV 内点找类似 → ephemeral 结果焦点/方向键浏览/空格开图跟修改前一致；**进结果时不应凭空多个默认高亮**（M2 传 `defaultHighlightFirst=false`）；M2 similar 网格回车现在也能开图（已授权的 M2 行为变化）

### V2 M3/M2 — ephemeral 结果开图错位修复（待真机验，`<pending>`）

- [ ] (2026-06-06 / `<pending>`) **找类似 V1 模式开图不错位**：从左侧**普通文件夹**的图进 QV → 点找类似 → 结果网格方向键移高亮/空格/双击开图 → QV 显示的图和缩略图**一致**（修前从普通文件夹进会错位）
- [ ] (2026-06-06 / `<pending>`) **搜索 V1 模式开图不错位**：普通文件夹模式下 ⌘F 搜索 → 结果网格开图 → 显示一致（同根问题一并修）
- [ ] (2026-06-06 / `<pending>`) **回归 — 智能文件夹模式不变**：从「全部最近」等智能文件夹进 QV 找类似/搜索开图仍正确（原本就对的路径不退化）
- [ ] (2026-06-06 / `<pending>`) **⌘F-from-preview**：V1 单击进 preview 看图时按 ⌘F → preview 关闭、进搜索 overlay，不残留错图
