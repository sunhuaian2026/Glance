...
### 重复清理 V2 重设计 — 任务 AB/C/D/E 真机验(2026-06-19 ship 待真机验)

> 任务 AB ship `147f318`，任务 C ship `3f1ca0d`，任务 D ship `419b4d0`~`20c4180`~`a2d2829`~`298029d`，任务 E ship `0b0d4c2`。verify.sh 三段全过(build 0 error 0 warning)，subagent-driven 多轮 self-fix。新建 4 文件（DedupListFilter / DedupSortOption / DedupCleanupV2View / DedupFocusReviewOverlay）+ 删 1 文件（DuplicateOverviewView）+ 改 4 文件（DuplicateOverviewModel / DuplicateGroup / DesignSystem / ContentView）。

军哥本机验项（任务 AB 7 项）:
- [ ] (2026-06-19 / `147f318`) **主视图 4 区域对得上**: 进「重复清理」侧边栏入口 → 顶栏（标题「重复清理」+ 「移入废纸篓 (N)」按钮）+ 汇总条（大数字组数/可省空间 + 双徽标「✓ N 张已自动」/「⚠ N 组待确认」 + 「逐组审阅 ›」按钮）+ 工具条（三个筛选 pill「全部/待确认/已自动」 + 排序「可省/张数/名称」 + 搜索框）+ 列表（叠放缩略图 + 组标题 + 删 N 徽）对得上设计稿
- [ ] (2026-06-19 / `147f318`) **展开区缩略图 + per-item 切保留张**: 点叠放缩略图或 chevron → 展开区显示 90×66 横排缩略图；点任一缩略图 → 该图变保留张（蓝色描边 + 「保留」badge）；再点另一张 → 保留态转移
- [ ] (2026-06-19 / `147f318`) **跳过按钮**: 点「跳过」按钮 → 整行 opacity 0.55 + 「删 N」变「已跳过」；再次进入总览（重启或重扫）跳过状态消失（session 态，D-dedup-2）
- [ ] (2026-06-19 / `147f318`) **筛选 pills 切换**: 点「待确认」→ 列表只显 needsReview 且未审阅的组；点「已自动」→ 只显 !needsReview 或已审阅的组；点「全部」→ 恢复全列表
- [ ] (2026-06-19 / `147f318`) **排序切换**: 点「张数」→ 副本数多的组排前；点「名称」→ 按文件名字母序；点「可省」→ 可省字节多的排前
- [ ] (2026-06-19 / `147f318`) **搜索文件名**: 搜索框输入「card」→ 列表只显 canonical 文件名含「card」的组；清空搜索框 → 恢复全列表
- [ ] (2026-06-19 / `147f318`) **移入废纸篓流程不变**: 勾组 + 点「移入废纸篓 (N)」→ 既有引导 sheet（V1 bookmark）或直接 trash（V2 bookmark）+ banner + undo 流程不变

军哥本机验项（任务 C 5 项）:
- [ ] (2026-06-19 / `3f1ca0d`) **duplicateCount=2 归「已自动」**: 两张副本的组显绿色「✓ 已自动留最优」徽章；汇总条「✓ N 张已自动」计数包含这些组
- [ ] (2026-06-19 / `3f1ca0d`) **duplicateCount>=3 归「待确认」**: 3 张或以上副本的组显黄色「⚠ 建议你确认」徽章；汇总条「⚠ N 组待确认」计数正确
- [ ] (2026-06-19 / `3f1ca0d`) **「已确认」按钮 / 浮层 Enter 后组迁移**: 点组行「已确认」按钮（或浮层 Enter）→ 该组从「待确认」迁到「已自动」徽章（汇总条 autoCount +1 / reviewCount -1）
- [ ] (2026-06-19 / `3f1ca0d`) **筛选「待确认」精准**: 工具条选「待确认」→ 只显 needsReview=true 且 reviewed=false 的组
- [ ] (2026-06-19 / `3f1ca0d`) **筛选「已自动」精准**: 工具条选「已自动」→ 同时显 needsReview=false（两张副本组）和 reviewed=true（用户已确认的大组）

军哥本机验项（任务 D 6 项）:
- [ ] (2026-06-19 / `20c4180`) **浮层弹出 + 焦点 ring**: 点「逐组审阅 ›」→ 浮层从主区 ZStack 淡入弹出（fade in + 0.96 scale 动画）；焦点 ring 在浮层内（键盘事件优先浮层，主区背景 grid 无响应）
- [ ] (2026-06-19 / `20c4180`) **← → 切组**: 浮层内按 ← → 键 → 左右切换队列里「待确认且未审阅且未跳过」的组；进度显示「i / N」正确更新
- [ ] (2026-06-19 / `20c4180`) **Enter 确认 + 自动推进**: 浮层内按 Enter → 当前组从「待确认」迁「已自动」→ 自动推进到队列下一组；最后一组确认后浮层自动关闭
- [ ] (2026-06-19 / `20c4180`) **Esc 关浮层**: 浮层内按 Esc → 浮层淡出关闭；焦点归还主区（可继续用键盘操作列表）
- [ ] (2026-06-19 / `20c4180`) **click 外部背景关浮层**: 点浮层外的半透明蒙层背景 → 浮层关闭；点浮层本体内区域 → 不关闭（D-dedup-7）
- [ ] (2026-06-19 / `20c4180`) **大图 2 列对比**: 浮层内显示 2 列大图（保留张 + 当前组某副本）；保留张有蓝色描边 + 「保留」标；点任意展开区缩略图可切换保留张（实时更新高亮）

军哥本机验项（任务 E 2 项）:
- [ ] (2026-06-19 / `0b0d4c2`) **浮层动画平滑**: 点「逐组审阅 ›」→ 浮层 fade in + 0.96 scale 动画平滑无抖；Esc 或 click-outside 关闭 → fade out 动画平滑
- [ ] (2026-06-19 / `0b0d4c2`) **trashing 期间浮层 disabled + reload 后状态**: 触发「移入废纸篓」开始 trashing 时，浮层若已打开 → 全 disabled（`.allowsHitTesting(false)` + 0.5 opacity）；trash 完 reload 后浮层队列重算（部分组消失后 index 自动推进）；若 reload 后队列空则浮层自动关闭
