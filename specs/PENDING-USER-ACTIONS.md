...
- [x] (2026-06-03 / `<本次>`) **现有残留自愈**：启动新版后，之前侧边栏移除过但缩略图仍残留的文件夹的图，从智能文件夹消失（启动 DB+bookmark 对账清掉）✓ 2026-06-03
- [x] (2026-06-03 / `<本次>`) **⚠️ 安全 — 在管文件夹未被误删**：仍在管理的文件夹的图全在，多文件夹重启后全部仍在（Guard B 没在启动瞬态误删整库）✓ 2026-06-03
