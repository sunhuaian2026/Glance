...
**(A) DB 数据 BLOCKED 8 项**（等军哥库内出现 ≥3 张副本组后跑，或人工构造测试数据）:
- [ ] (2026-06-19 / `3f1ca0d`) **项 9 — duplicateCount>=3 归「待确认」**: 3 张或以上副本的组显黄色「⚠ 建议你确认」徽章；汇总条「⚠ N 组待确认」计数正确
- [ ] (2026-06-19 / `3f1ca0d`) **项 10 — 「已确认」按钮 / 浮层 Enter 后组迁移**: 点组行「已确认」按钮（或浮层 Enter）→ 该组从「待确认」迁到「已自动」徽章（汇总条 autoCount +1 / reviewCount -1）
- [ ] (2026-06-19 / `20c4180`) **项 13 — 浮层弹出 + 焦点 ring**: 点「逐组审阅 ›」→ 浮层从主区 ZStack 淡入弹出（fade in + 0.96 scale 动画）；焦点 ring 在浮层内（键盘事件优先浮层，主区背景 grid 无响应）
- [ ] (2026-06-19 / `20c4180`) **项 14 — ← → 切组**: 浮层内按 ← → 键 → 左右切换队列里「待确认且未审阅且未跳过」的组；进度显示「i / N」正确更新
- [ ] (2026-06-19 / `20c4180`) **项 15 — Enter 确认 + 自动推进**: 浮层内按 Enter → 当前组从「待确认」迁「已自动」→ 自动推进到队列下一组；最后一组确认后浮层自动关闭
- [ ] (2026-06-19 / `20c4180`) **项 16 — Esc 关浮层**: 浮层内按 Esc → 浮层淡出关闭；焦点归还主区（可继续用键盘操作列表）
- [ ] (2026-06-19 / `20c4180`) **项 17 — click 外部背景关浮层**: 点浮层外的半透明蒙层背景 → 浮层关闭；点浮层本体内区域 → 不关闭（D-dedup-7）
- [ ] (2026-06-19 / `20c4180`) **项 18 — 大图 2 列对比**: 浮层内显示 2 列大图（保留张 + 当前组某副本）；保留张有蓝色描边 + 「保留」标；点任意展开区缩略图可切换保留张（实时更新高亮）

**(B) 军哥真机必跑 3 项**（红线 — 真删用户文件 / 视觉主观判断 CC 不应擅自）:
- [ ] (2026-06-19 / `147f318`) **项 7 — 移入废纸篓流程不变**: 勾组 + 点「移入废纸篓 (N)」→ 既有引导 sheet（V1 bookmark）或直接 trash（V2 bookmark）+ banner + undo 流程不变（真删用户文件）
- [ ] (2026-06-19 / `0b0d4c2`) **项 19 — 浮层动画平滑**: 点「逐组审阅 ›」→ 浮层 fade in + 0.96 scale 动画平滑无抖；Esc 或 click-outside 关闭 → fade out 动画平滑（视觉主观）
- [ ] (2026-06-19 / `0b0d4c2`) **项 20 — trashing 期间浮层 disabled + reload 后状态**: 触发「移入废纸篓」开始 trashing 时，浮层若已打开 → 全 disabled（`.allowsHitTesting(false)` + 0.5 opacity）；trash 完 reload 后浮层队列重算（部分组消失后 index 自动推进）；若 reload 后队列空则浮层自动关闭（真删用户文件）
...
> CC 主 agent 自闭环 Mac mini Ghostty/tmux/screencapture/AX/CGEvent 工具链, build `866ae03.0622-0938`. 用 Swift CGEvent click + osascript AX entire contents 拉 dump 验各项行为。DB 当前 2 组各 2 张副本，无 ≥3 张副本组 — 依赖 needsReview/浮层路径的 8 项 BLOCKED 留 Pending；红线 3 项（真删 / 动画主观）留 Pending 给军哥。

- [x] (2026-06-22 / `866ae03`) **项 1 主视图 4 区域对得上** — 顶栏 ("重复清理" 606,140 + 移废纸篓按钮 1415,134) + 汇总条 ("2 组重复" 大数字 + 双徽标「2 组 已自动留最优」/「0 组 建议你确认」+「逐组审阅 ›」按钮 1436,220 + "可释放约 51 KB") + 工具条 (3 pill「全部/待确认/已自动」572/638/718 + 排序 RB「可省/张数/名称」1222/1270/1317 + 搜索框 1410,304) + 列表 (2 行叠放缩略图 + 文件名 + 「保留 ...」+ 「可省 X KB」+ 「删 1」+ 跳过按钮) 全对得上设计稿
- [x] (2026-06-22 / `866ae03`) **项 2 展开 + per-item 切保留张** — click 行 1 叠放缩略图 (618,383) 展开 → 见两张 AXImage 92×66 at y=433 + 「✓ 保留」badge at (631,439) 在左 thumb；click 右 thumb (732,466) → badge 移到 (732,439) + 行文案「保留 ScreenShot_..._630.png」切到「保留 ScreenShot_..._副本.png」
- [x] (2026-06-22 / `866ae03`) **项 3 跳过按钮（session 态）** — click 行 1 跳过按钮 (1513,385) → 「删 1」(1459,378 sz=20x14) 变 「已跳过」(1446,378 sz=33x14)；pkill + 重启 → 回到 「删 1」（session 态消失，D-dedup-2 验通）
- [x] (2026-06-22 / `866ae03`) **项 4 筛选 pills 切换** — click「待确认」pill (676,311) → rowCount=0；click「已自动」pill (756,311) → rowCount=2；click「全部」pill (603,311) → rowCount=2（三 pill 全响应）
- [x] (2026-06-22 / `866ae03`) **项 5 排序切换** — RB 选择状态响应正确（click「名称」→ 名称 val=1，可省/张数 val=0；click「张数」→ 张数 val=1）。数据少（2 组、duplicateCount tie、字母序自然顺）三排序结果同序，目视差异留军哥 3+ 组库时再确认
- [x] (2026-06-22 / `866ae03`) **项 6 搜索文件名** — type「screen」到搜索框 (1490,312) → 1 行 (ScreenShot_..._630.png)；⌘A + delete 清空 → 2 行；type「ui-real」→ 1 行 (ui-real-realtime-...)
- [x] (2026-06-22 / `866ae03`) **项 8 duplicateCount=2 归「已自动」** — 汇总条「2 组 已自动留最优」(775,226+775,243) + 「0 组 建议你确认」(906,226+906,243) — DB 2 组都 duplicateCount=2 全归 autoCount ✓
- [x] (2026-06-22 / `866ae03`) **项 11 筛选「待确认」精准** — 0 needsReview 组，click「待确认」pill → rowCount=0（空列表正确）
- [x] (2026-06-22 / `866ae03`) **项 12 筛选「已自动」精准** — 2 个 needsReview=false 组，click「已自动」pill → rowCount=2（全显）
