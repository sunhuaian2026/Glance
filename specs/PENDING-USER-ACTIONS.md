...
- [ ] (2026-06-03 / `<本次>` / 方案 3 Slice 2) **加载失败显占位**：自然遇到"图在索引里但读不到"时（真删磁盘文件的 FSEvents race / 损坏文件 / 不支持格式），grid cell / 预览 / QV 应显「无法加载」占位（photo.badge.exclamationmark）而非无限转圈。难按需复现，自然遇到时确认。可人工触发：受管文件夹放损坏图（.txt 改名 .jpg）或 QV 开着时 Finder 删当前图
...
- [x] (2026-06-03 / `<本次>`) **回归 — 正常加载未坏**：正常图片在 grid / 预览 / QV 都正常加载显示（`nonisolated` 解码改动 + loadFailed 标志未破坏正常路径）✓ 2026-06-03
