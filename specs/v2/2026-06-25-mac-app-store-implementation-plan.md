...
## 11. 2026-06-26 ASC 后台填表暂停档(任务 3 partial)

军哥决策:**暂停 1.0 海外 ship,先办中国大陆 ICP + App 备案,办齐再统一推进**。原 design Section 8 推荐路径 A(1.0 海外先 ship + 1.1 补中国大陆)→ 改为路径 B(等齐一并 ship)。理由:个人开发者 + 小红书 382336617 用户主要在中国大陆,不想分两批 ship。

### 11.1 ASC 后台填表进度快照(截止 2026-06-26)

本 session(2026-06-26)CC 全程指导军哥 ASC 后台填表,完成大部分字段,卡在 ICP 备案号 + Build + 截屏三处阻塞项。

#### 综合 → App 信息

| 字段 | 状态 | 内容 |
|---|---|---|
| 名称 | ✓ 已填 | 一眼 Glance(21 字符) |
| 副标题 | ✓ 已填 | 本地看图、识图、去重(D6.1) |
| 套装 ID | ✓ 锁定 | Glance - com.sunhongjun.glance |
| SKU | ✓ 锁定 | glance-macos |
| Apple ID(自动) | ✓ | 6784497208 |
| 主要语言 | ✓ | 简体中文 |
| 主要类别 | ✓ | 工具(实用工具) |
| 次要类别 | ✓ | 摄影与录像 |
| 内容版权 | ✓ | 否(不含/不显示/不访问第三方内容) |
| 许可协议 | ✓ | Apple 标准许可协议 |
| 年龄分级 | ✓ | 4+(7 步问卷全最低档,172 国 4+ / 巴西 AL / 韩国 全部 / 越南 00+) |
| 适龄信息 URL | ✓ | 留空 |
| 加密文稿 | ⚠ 未填 | 计划走 Info.plist `ITSAppUsesNonExemptEncryption=NO` 一劳永逸(D6.4,恢复时 CC commit) |
| 数字服务法(DSA) | ✓ | non-trader 默认(D6.3,跳欧盟) |
| 中国大陆 ICP 备案号 | ❌ 阻塞 | 等备案到手填(沪/京/鲁 ICP 备 XXXXX 号-1A 格式) |
| 越南游戏许可 | ✓ | 跳(不是游戏) |
| App Store 服务器通知 | ✓ | 跳(无 IAP/订阅) |
| App 专用共享密钥 | ✓ | 跳(无 IAP) |
...
|---|---|---|
| App 预览(视频) | ✓ | 留空 0/3 |
| App 截屏 | ❌ 阻塞 | 0/10,等任务 6 截 7 张 2880×1800 |
| 推广文本 | ✓ | 留空(可后改无需重审) |
| 描述 | ✓ 已填 | 纯净版无 ✦/• 装饰符(描述字段拒收 Dingbats 块字符,改全文本)~3589 字符 |
| 关键词 | ✓ 已填 | 看图,看图器,图片浏览,本地图库,重复图片,去重,找相似,EXIF,文件夹,缩略图 |
| 技术支持网址 | ✓ 已填 | https://sunhuaian2026.github.io/Glance/ |
| 营销网址 | ✓ 已填 | 同上 |
| 版本 | ✓ | 1.0 |
| 版权 | ✓ 已填 | © 2026 孙红军 |
| 构建版本 | ❌ 阻塞 | 等任务 7 altool 上传 `Glance.pkg` 后从 Build 列表选 |
| Game Center | ✓ | 不勾 |
| App 审核信息 - 需要登录 | ✓ | 不勾 |
| App 审核信息 - 联系信息 | ⚠ 未填 | 待军哥填(姓孙/名红军/电话/邮箱 `16414766@qq.com`) |
| App 审核信息 - 备注 | ✓ 已填 | 4 点说明(无服务端 / 测试建议 / 沙盒 trash API / 隐私承诺 URL) |
| App 审核信息 - 附件 | ✓ | 跳 |
| App 沙盒信息 | ✓ | 留空(entitlements 全是 Apple 默认信任授权类型) |
| App Store 版本发布 | ⚠ 未确认 | D6.5 推荐「手动发布此版本」(留小红书预热缓冲) |

#### App Store → App 隐私

| 字段 | 状态 | 内容 |
|---|---|---|
| Privacy Nutrition Labels | ✓ 已发布 | 数据收集=否 + 跟踪=否(产品页面预览「未收集数据」绿色 ✓) |
| 隐私政策网址 | ⚠ 未填 | `https://sunhuaian2026.github.io/Glance/privacy.html`(待编辑 + 发布) |
| 用户隐私选择网址 | ✓ | 留空(可不填) |
...
|---|---|
| 价格 | ❌ 未决 | ¥6 或免费(等军哥拍) |
| 销售范围 | ❌ 阻塞 | 等中国大陆备案齐 → 全球(含中国大陆) |

### 11.2 本 session 关键决策记录

- **D6.1 副标题** "本地看图、识图、去重"(候选 5,军哥钦定)
  - 弃 "看图、找重复、省硬盘"("省硬盘"too low)
  - 选 "本地看图、识图、去重" — 突出 "识图" 技术感(SHA256 + Vision)+ 本地特征 + 双卖点
- **D6.2 备案路径切 B** — 弃路径 A(分两批)→ 选路径 B(等齐一并 ship)。理由:中国大陆是核心用户群,小红书 382336617 已有粉丝基础,不想分两批宣传
- **D6.3 DSA Trader** non-trader 默认保持(欧盟不显示,个人开发者不上传营业执照)
- **D6.4 加密合规** 走 Info.plist `ITSAppUsesNonExemptEncryption = NO`(一劳永逸 代替每次 build 手动声明)— **待 CC commit**(恢复时)
- **D6.5 App Store 版本发布** 「手动发布此版本」(审核通过后军哥手动按钮,留小红书预热缓冲)
- **D6.6 副标题字符** ASC 描述字段拒收 ✦(U+2726 Dingbats) + •(U+2022 项目符号),改用纯文本中文标点。沉淀:ASC 文案禁用 Dingbats 块字符

### 11.3 暂停 + 恢复条件

**暂停原因**:中国大陆 ICP + App 备案(35-45 日历日)是必要前置,先办齐再统一推进剩余任务,避免分批 ship 宣传分散。

**恢复条件 ALL of**:
1. 中国大陆 App 备案号到手(沪/京/鲁 ICP 备 XXXXX 号-1A 格式)
2. 备案绑定的可备案域名 + 国内云服务器持续可用(避免后续年度审核掉链)
3. PENDING `Mac App Store 上架 — ICP + App 备案` sub-task 全部勾掉

### 11.4 恢复后剩余 ASC 动作(按顺序)

1. CC commit Info.plist `ITSAppUsesNonExemptEncryption = NO`(D6.4)+ verify.sh + push
2. ASC **App 信息 → ICP 备案号** 填入 → 保存
3. ASC **App 信息 → 加密文稿** 状态自动豁免(因步骤 1 生效)
4. ASC **App 隐私 → 隐私政策网址** 填 `https://sunhuaian2026.github.io/Glance/privacy.html` → 发布
5. ASC **macOS App 1.0 → App 审核信息 → 联系信息** 填军哥联系方式
6. ASC **macOS App 1.0 → App Store 版本发布** 勾「手动发布此版本」
7. ASC **营利 → 价格与销售范围** 设价格(¥6 或免费)+ 销售范围(全球含中国大陆)
8. **任务 6** 装 `.pkg` → 准备测试图库 → 截 7 张 2880×1800 PNG → 上传 ASC「App 预览和截屏」
9. **任务 7** `make release-appstore` 重新打包(确保 Info.plist 加密 key 生效)→ `altool` 上传 `.pkg` → ASC 关联 Build → 提交审核 → 等 1-3 天 → 通过后手动发布
10. **任务 9** Roadmap + CONTEXT.md + 术语表同步 + plan 偏差回写
