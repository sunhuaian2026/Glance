...
└── Glance/                         ← Swift 源码（PBXFileSystemSynchronizedRootGroup，新文件自动加入编译）
    ...
    ├── ContentView.swift            ← NavigationSplitView (sidebar VStack: SmartFolderListView + V1 FolderSidebarView) + mainContent ZStack(baseGrid + previewOverlay) + QuickViewer（**快速看图器工具栏 任务 1 2026-06-08：从 .overlay 迁到 MainQuickViewerWindowController 独立无装饰窗**，盖主窗修 toolbar regression；ContentView 仅 @ObservedObject 观察 isPresenting 决定 allowsHitTesting/previewOverlay 渲染，进出走 presentQuickViewer/handleQVDismiss，退出按 QVDismissalReason{normal/findSimilar/commandF} 仲裁）；mainContent .allowsHitTesting(QV 不在时) 让底层 grid tooltip tracking 在 QV 期失活。**方向2 任务 2 已删整套 OpenWith externalOpen 残留机器**（externalOpenUrls/handleExternalOpen/scheduleActivation/waitForAppActivation/QuickViewerEntry.externalOpen/onChange(pendingOpen,windowIdentity,isWindowKey)/handleBrowseFolder）——外部打开改走 ExternalViewerWindowController 独立看图窗，ContentView 不再参与；**WindowAccessor 也移除**（图库主窗 NSWindow 挂接改由 MainWindowController 自任 delegate 接管，D-OW16）。**M4 任务 1 步骤 4 集成**：加 `@StateObject duplicateOverviewModel = DuplicateOverviewModel.placeholder()` + `@State showDuplicateOverview: Bool`；主区 `.environmentObject(duplicateOverviewModel)` 注入（侧边栏不注入，避免选中态漂移）；SmartFolderListView 调用传 `isDuplicateOverviewSelected: showDuplicateOverview` + `onSelectDuplicates` callback；mainContent ZStack `showDuplicateOverview` 分支 swap `DedupCleanupV2View()`（与临时结果/baseGrid 互斥，最优先分支）；**五态互斥**`.onChange` 扩展（V1 folder / 智能文件夹 / 临时结果 / 搜索 overlay / 重复清理总览）— `.onChange(of: showDuplicateOverview)` 是 load 唯一 owner + 进入总览时清其它四态；进入临时结果/搜索 overlay 时清 showDuplicateOverview（handleFindSimilar 尾部 + openSearch 内）；`wireIfReady` 调 `duplicateOverviewModel.attach(indexStore:bridge:)`。**M4 任务 2 收尾 step A.5 集成**：加 `@StateObject migrationCoordinator = BookmarkMigrationCoordinator.placeholder()` + `@EnvironmentObject bookmarkManager`；`wireIfReady` 修 `attach()` callsite 扩展为 5 参数 (indexStore/bridge/bookmarkManager/folderStore/migrationCoordinator)；NavigationSplitView **外层** modifier 链 `.sheet(isPresented: $migrationCoordinator.isPresenting)` 接 `BookmarkMigrationView(onConfirm: pickRoots, onDismiss: cancel)`；`.onChange(of: duplicateOverviewModel.groups)` prune selectedSha256s（D5-bm-ui — 重扫完总览 reload 后把不在新 groups 里的 sha256 从勾选集合移除，避免 stale 状态）；detail closure 加 `.environmentObject(migrationCoordinator)` 让 DedupCleanupV2View 拿到 phase 渲染重扫中专用空态。**2026-06-18 followup**: `.toolbar` 在「信息 ⌘I」和「外观切换」之间插查找按钮(`DS.Icon.search` magnifyingglass), 点击 = `openSearch()` 跟 ⌘F 同效果; 故意**不挂 `.keyboardShortcut("f")`** 避免与 body 末尾 `.onKeyPress(.init("f"))` 双绑触发两次, ⌘F 仍走原 `.onKeyPress` 唯一处理, button 只当点击入口 + `.help("查找 (⌘F)")` tooltip 显快捷键。**重复清理 V2 任务 AB-E 改动**：`AppFocus` 加 `.dedupOverlay` case（D-dedup-15，焦点仲裁链接入）；主区 ZStack 接 `DedupFocusReviewOverlay` overlay 层；五态互斥 `.onChange` 扩展覆盖浮层焦点归还；prune 块（进临时结果/搜索 overlay 时清 dedupOverlay 焦点）
    ├── DesignSystem.swift           ← DS.Spacing / DS.Color / DS.Anim 等所有 UI 常量；**M4 任务 1 步骤 2 加 `enum Dedup`**（reloadDebounceMillis=500 / groupRowSpacing=DS.Spacing.lg / groupCellThumbnailSize=96 / groupCellThumbnailMaxPixel: Int = 192 / canonicalBadgeColor: SwiftUI.Color = .green / statsBarFont / emptyStateFont / duplicateThumbnailOpacity=0.7）；**重复清理 V2 任务 AB-E 加 `enum Dedup` ~50 新常量**（顶栏 topBarHeight / 汇总条 summaryCountFont/summaryBadge*/focusReviewButtonFont / 工具条 filterPillFont/sortSegmentFont / 列表行 listRowHeight/stackThumbnailSize/stackThumbnailOffset/rowTitleFont/deleteBadgeFont/skipButtonFont / 展开区 expandedThumbnailSize/expandedThumbnailCornerRadius/keepBadgeFont / 浮层 overlayMaxWidth/overlayMaxHeight/overlayCornerRadius/overlayBigThumbSize/overlayBigThumbCornerRadius/overlayProgressFont/overlayTitleFont/overlayKeepBadgeFont/overlayFilenameFont/overlayPathFont / 以及 needsReviewThreshold=3 等行为常量）
    ...
    │   ├── SmartFolderListView.swift    ← V2 sidebar 智能文件夹区（M1 "全部最近" + 后续 "本周新增"）；**M4 任务 1 步骤 4 加 isDuplicateOverviewSelected: Bool 参数 + onSelectDuplicates callback + 独立私有 DuplicateCleanupRow** (不混进 availableSmartFolders ForEach；选中态 ContentView 持 showDuplicateOverview 唯一权威传入；trash icon + 「重复清理」labels + accent 选中态背景)
    ...
    ├── Migration/                  ← V2 M4 任务 2 收尾 — V1→V2 bookmark 升级引导 UI（2026-06-17 ship）
    │   ├── BookmarkMigrationView.swift       ← SwiftUI sheet 纯展示（标题「升级清理权限」+ 三句话主文案 +「以后再说」/「重新选择根目录 →」两按钮 + DisclosureGroup 默认折起「为什么需要重新选?」单行技术解释「macOS 沙盒授权模型限定：只读 bookmark 不能升级为读写，必须重新创建」）；入参 onConfirm + onDismiss，本 view 不持状态；D1-bm-ui SwiftUI .sheet 形态 + D6-bm-ui 三句话简洁 + DisclosureGroup 折起
    │   └── BookmarkMigrationCoordinator.swift ← @MainActor ObservableObject 状态机单一权威（phase: idle/presentingSheet/picking/rescanning/completed）；weak refs (model/bookmarkManager/folderStore/bridge) 防循环引用；start(model:bookmarkManager:folderStore:bridge:) — design 偏差修正加 bridge 参数（原 design 假设 folderStore.bridge 但实际 bridge 在 ContentView @State 持）+ cancel() + pickRoots() async；pickRoots 持久化提交段 4 步串行（clearAllForMigration + reloadFromDefaults + addFolders + 一次性 bridge.addIndexChangedObserver 等首次入库 → markSchemaV2 + phase=.completed → .idle）；NSOpenPanel allowsMultipleSelection=true + 文案明示「替换全部根目录漏选会丢失」(codex P1 修(漏选)); 无 sleep 魔数 (codex P1 修(魔数)); 失效降级三段 guard log + reset .idle (codex P2 修(生命周期)); 失败走 V2 既有 bridge.lastError banner sheet 不重弹 (codex P1 修(失败时序))
    └── Dedup/                       ← V2 M4 重复清理（去重省空间，回归初心闭环）
        ├── DuplicateGroup.swift              ← M4 任务 1 步骤 2：4 个值类型 record — DuplicateGroup（id=sha256 / canonical 保留张 / duplicates 副本数组 / reclaimableBytes 可省空间）+ DuplicateGroupMember（id / urlBookmark / relativePath / fileSize / fullPath / isCanonical — 不含 folderId 任务 2 才扩）+ DuplicateGroupRow（聚合查询返回行 sha256/memberCount/reclaimableBytes）+ DuplicateGroupMemberRow（成员明细查询返回行 id/dedupCanonical/fileSize/relativePath/urlBookmark/fullPath）；**重复清理 V2 任务 AB 扩展**：extension DuplicateGroup 加 `allMembers`（canonical + duplicates 平铺）/ `recommendedKeepId`（= canonical.id，即 DedupPass 算法 earliest birth_time + 最小 id，SHA256 invariant 下无体积差异）/ `duplicateCount`（副本数，`needsReview` 算法唯一判别维度）；`nonisolated` 修饰关键——DuplicateGroup 是值类型在 Swift 6 actor isolation 下的跨 actor 传递需 `nonisolated` computed properties
        ...
        ├── DuplicateOverviewModel.swift      ← M4 任务 1 步骤 3：@MainActor ObservableObject 单一 @Published state + placeholder()/attach(indexStore:bridge:) 装配 + load() Task.detached fetchGroups + generation counter stale-write guard + scheduleReload() DispatchWorkItem debounce 500ms + bridge.addIndexChangedObserver token（寿命跟 model 一致不持 detach）+ computed accessors（groups/isLoading/errorMessage/groupCount/totalReclaimableBytes）；private nonisolated static fetchGroups/makeMember 让 detached Task 可调（@MainActor class 内 static 默认继承隔离需显式 nonisolated）。**M4 任务 2 收尾 step A.4 增量**：加 3 weak ref 依赖（bookmarkManager / folderStore / migrationCoordinator）+ attach 扩展签名加 3 新依赖 + trashSelected 入口加 schemaVersion >= 2 guard 触发 migrationCoordinator.start (V1 路径 return 不走 trash 主流程) + 新 API replaceSelectedSha256s(_:) 一次性替换 (D5-bm-ui prune 用)。**重复清理 V2 任务 AB 重构**：`selectedSha256s → skippedGroupIds`（跳过组 session 态 Set，不写 DB）；新增 `userKeepIdByGroup: [String: Int64]`（per-item 选保留张临时态）；新增 `focusReviewQueue/focusReviewIndex/isFocusReviewOpen` 逐组审阅浮层状态；`trashSelected` 语义改 `trashPending`（D1 临时态语义，获取 pending 副本列表）；prune 块（reload 后 skippedGroupIds/reviewedGroupIds/expandedGroupIds/focusReviewQueue/focusReviewIndex 全部 prune 清出不在新 groups 的 stale 状态）；新增 `focusReview*` 方法（openFocusReview/closeFocusReview/moveToNextInQueue/markCurrentReviewed）
        ├── DedupListFilter.swift             ← 重复清理 V2 任务 AB：工具条筛选 pills 三档值类型 enum（`.all` 全部 / `.needsReview` 待确认 / `.reviewed` 已自动），用于 DedupCleanupV2View 工具条 pill 切换 + DuplicateOverviewModel 过滤逻辑
        ├── DedupSortOption.swift             ← 重复清理 V2 任务 AB：排序分段三档值类型 enum（`.reclaimable` 可省 / `.count` 张数 / `.name` 名称），用于 DedupCleanupV2View 工具条 Picker + DuplicateOverviewModel 排序逻辑
        ├── DedupCleanupV2View.swift          ← 重复清理 V2 主视图（任务 AB-E 全覆盖）：顶栏（标题「重复清理」+ 「移入废纸篓 (N)」主操作按钮）+ 汇总条（大数字 N 组/可省 X + 双徽标「✓ N 张已自动」/「⚠ N 组待确认」 + 「逐组审阅 ›」按钮）+ 工具条（筛选 pills + 排序 Picker + 搜索框）+ 列表（LazyVStack 全量渲染，D-dedup-5）；私有 DedupGroupRow 渲染单行（叠放缩略图 + 组标题 + 删 N 徽 + needsReview ⚠ 徽 + 跳过按钮 + chevron 展开）；私有 DedupExpandedArea 渲染 90×66 缩略图横排（点任一缩略图 = 设保留张临时态，无 ★ 推荐角标）；空态 checkmark.seal + 错态 exclamationmark.triangle；`@EnvironmentObject migrationCoordinator` 接 rescanning 专用空态
        ├── DedupFocusReviewOverlay.swift     ← 重复清理 V2 逐组审阅浮层 view（任务 D）：ZStack overlay + `.ultraThinMaterial` 背景蒙层（D-dedup-6）；背景层 `.onTapGesture { model.closeFocusReview() }` 实现 click-outside 关闭（D-dedup-7）；对话框本体 `.contentShape(Rectangle())` 拦截内部点击不冒泡；大图 2 列（保留张 + 当前副本，各 300×214 上限）；键盘 ← → 切组 + Enter 确认当前 + Esc 关闭（`AppFocus.dedupOverlay` 焦点仲裁，D-dedup-15）；进度「i / N」显示；任务 E 加 fade in/out + 0.96 scale 动画 + trashing 期间 `.allowsHitTesting(false)` + 0.5 opacity disabled 态
        ├── IndexedImageSnapshot.swift        ← M4 任务 2 步骤 2.1：15 字段值类型快照（id/urlBookmark/birthTime/fileSize/format/filename/relativePath/folderId/dimensionsWidth/dimensionsHeight/contentSHA256/dedupCanonical/featurePrintRevision/supportsFeaturePrint/exifCaptureDate）— 删前 SELECT 全保真,撤销 INSERT 15 列回补不丢字段；Sendable + Hashable for Set/cross-actor
        ├── TrashOutcome.swift                ← M4 任务 2 步骤 3.1：4 个值类型 record — TrashOutcome（successes/failures/cancelled + computed successCount/failureCount）+ TrashSuccess（filename/relativePath/groupKey + 重组 trashURL 用）+ TrashFailure（filename/relativePath/reason 错误码字符串）+ RestoreOutcome（successes/failures/cancelled + 同 computed）；TrashCancellationToken actor（cancel + isCancelled，跨任务安全）；全标 Sendable
        ├── TrashService.swift                ← M4 任务 2 步骤 3.2+3.3：全项目首个**碰真实文件**服务（仅本服务调 FileManager.trashItem/moveItem）。trashItems(snapshots:cancellationToken:progress:) — 复刻 DedupPass.computeSha 沙盒 scope 模式：resolve urlBookmark + startAccessingSecurityScopedResource + trashItem(at:resultingItemURL:) + 失败累积非中断 + cancel 短路 + 每 50 张回调 progress；restoreItems(successes:cancellationToken:progress:) — trashURL → 重组 origFullPath via composeOriginalFullPath(snapshot:) + moveItem(at:to:) 回原路径
        └── TrashUndoBanner.swift             ← M4 任务 2 步骤 5.2：ContentView 级 overlay banner 纯展示 view（D33 跨视图持久）。input TrashOutcomeEvent + onUndo + onDismiss; 双阶段 — trash 阶段（undoResult=nil）显「已移 N 张到废纸篓 [撤销] [×]」+ 副文案（失败数 / 已取消）; undo 阶段（undoResult≠nil）显「撤销完成 N 张」+ 副文案（撤销失败数）+ 不再显撤销按钮; .thinMaterial.opacity(0.92) 半透明背景 + 圆角 12 + 最大宽度 480; 状态由 ContentView 拥有,本 view 纯展示
```
