...
└── Glance/                         ← Swift 源码（PBXFileSystemSynchronizedRootGroup，新文件自动加入编译）
    ...
    └── Dedup/                       ← V2 M4 重复清理（去重省空间，回归初心闭环）
        ...
        ├── DuplicateOverviewView.swift       ← M4 任务 1 步骤 4：只读总览 UI（统计条「X 组重复 · 可省 Y」+ ScrollView+LazyVStack 组列表 + 空态 checkmark.seal「没找到重复图」+ 错态 exclamationmark.triangle）；私有 DuplicateGroupRowView 渲染单组（保留张 + 副本 + 组可省空间）；私有 DuplicateMemberCell 渲染单张图（保留张「保留」绿色 Capsule badge + 副本 opacity 0.7 弱化 + hover tooltip 完整路径）；loadThumb mirror DedupPass.computeSha scope 模式（resolve root bookmark + startAccessing + 拼 child URL + loadThumbnail nonisolated 顶层函数）；不持 .onAppear load 触发（ContentView .onChange(showDuplicateOverview) 是唯一 owner）；不持索引 chip（ContentView 全局 IndexingProgressView overlay 已渲染）。**M4 任务 2 步骤 5.3 增量**：DuplicateGroupRowView 加 `let isSelected: Bool + let onToggleSelection: () -> Void` + 顶部组级 Toggle(.checkbox)「选择此组清掉 N 张副本」（D28 整组勾选,不给单文件 checkbox）；statsBar 加 Spacer + trashAction @ViewBuilder（idle/completed 态显「移入废纸篓 (N 张)」borderedProminent 按钮 disabled when selectedSha256s 空 / trashing(done,total) 态换 ProgressView linear + done/total monospaced + 取消按钮）；groupsList ForEach callsite 透传 isSelected + onToggleSelection。**M4 任务 2 收尾 step A.6 增量**：加 @EnvironmentObject migrationCoordinator + mainContent 加 case .rescanning 分支最优先级 + rescanningState computed（ProgressView 圆形 + 「重新扫描中…」主文案 + 副文案「重选根目录后正在重建图像索引,扫完会自动显示重复组」）；D4-bm-ui 区别于 emptyState「没找到重复图」避免用户误读「重扫中」为「没找到」
